...
void HardwareSerial::begin(unsigned long baud, int uart_mode)
{
    ...
    if (uart_num < UART_COUNT)
    {
        ...
        HAL_UART_Receive_IT(((UART_HandleTypeDef *)this->huart_handle), _hal_buf, IT_LEN);
        _initalized = true;
    }
}
...
size_t HardwareSerial::write(uint8_t c)
{
    ...
    if (((UART_HandleTypeDef *)this->huart_handle)->gState != HAL_UART_STATE_BUSY_TX) {
       /*if (FifoReadChar(&tx_fifo,_hal_tx_buf)) {
            HAL_UART_Transmit_IT(((UART_HandleTypeDef *)this->huart_handle), _hal_tx_buf, 1);
        } */
        process_tx();
    }
#else
    HAL_UART_Transmit(((UART_HandleTypeDef *)this->huart_handle), &c, 1, 1000);
#endif
    return 1;
...
size_t HardwareSerial::write(const uint8_t *buffer, size_t size)
{
   ...
   // HAL_UART_Transmit_IT(((UART_HandleTypeDef *)this->huart_handle), data_ptr, size/*, size*100 */);
 #else
    HAL_UART_Transmit(((UART_HandleTypeDef *)this->huart_handle), data_ptr, size, size * 100);
 #endif   
    //printf("res = %d\n", result);
    //HAL_UART_Transmit(((UART_HandleTypeDef *)this->huart_handle), data_ptr, size, size * 100);
    return size;
...
void HardwareSerial::process_tx()
{
    ...
    if (len) HAL_UART_Transmit_IT(((UART_HandleTypeDef *)this->huart_handle), _hal_tx_buf, len);
}
#endif
...
void HardwareSerial::uart_init(unsigned long baud, int uart_mode)
{
    ...
    {
    case 0:
        ((UART_HandleTypeDef *)this->huart_handle)->Instance = UART0;
        break;
    case 1:
        ((UART_HandleTypeDef *)this->huart_handle)->Instance = UART1;
        break;
    case 2:
        ((UART_HandleTypeDef *)this->huart_handle)->Instance = UART2;
        break;
    case 3:
        ((UART_HandleTypeDef *)this->huart_handle)->Instance = UART3;
        break;
    case 4:
        ((UART_HandleTypeDef *)this->huart_handle)->Instance = UART4;
        break;
    case 5:
        ((UART_HandleTypeDef *)this->huart_handle)->Instance = UART5;
        break;
    }

    ((UART_HandleTypeDef *)this->huart_handle)->Init.BaudRate = baud;
    ((UART_HandleTypeDef *)this->huart_handle)->Init.WordLength = UART_WORDLENGTH_8B;
    ((UART_HandleTypeDef *)this->huart_handle)->Init.StopBits = uart_mode & UART_LC_STOP_Msk;
    ((UART_HandleTypeDef *)this->huart_handle)->Init.Parity = uart_mode & (~UART_LC_STOP_Msk);
    ((UART_HandleTypeDef *)this->huart_handle)->Init.Mode = UART_MODE_TX | UART_MODE_RX;
    ((UART_HandleTypeDef *)this->huart_handle)->Init.HwFlowCtl = UART_HWCONTROL_NONE;

    if (((UART_HandleTypeDef *)this->huart_handle)->gState != HAL_UART_STATE_RESET)
    {
        HAL_UART_DeInit(((UART_HandleTypeDef *)this->huart_handle));
    }

#if USE_UART0_PRINT
    int result = HAL_UART_Init(((UART_HandleTypeDef *)this->huart_handle));
    if (result != HAL_OK)
    {
...
#else
    HAL_UART_Init(((UART_HandleTypeDef *)this->huart_handle));
#endif
}
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
    ...
    if (huart->Instance == UART0)
    {
        ...
        HAL_NVIC_SetPriority(UART0_IRQn, 5);
    ...
    else if (huart->Instance == UART1)
    {
        ...
        uart_pins_init(huart);
        HAL_NVIC_SetPriority(UART1_IRQn, 5);
