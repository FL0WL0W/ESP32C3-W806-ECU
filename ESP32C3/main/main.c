...
{
    return uart_read_bytes(1, val, 1, pdMS_TO_TICKS(100)) > 0;
}

//write UPDI byte
bool UPDI_Write(uint8_t val)
{
    return uart_write_bytes(1, &val, 1) > 0;
}
...
void UPDI_Break()
{
    ...
    uart_wait_tx_done(1, pdMS_TO_TICKS(30)); //wait for all bytes to be flushed
    uart_get_baudrate(1, &baudrate);
    uart_set_baudrate(1, 10000);
    UPDI_Write(0);  // send a zero byte
    uart_wait_tx_done(1, pdMS_TO_TICKS(30)); // wait for 0 byte to finish before restore normal baudrate
    uart_flush_input(1);
    uart_set_baudrate(1, baudrate); // set baudrate back to normal after break is sent
}
...
extern esp_err_t gpio_od_enable(gpio_num_t gpio_num);

bool UPDI_Enable(gpio_num_t pin)
{
    //start UPDI
    gpio_set_level(pin, 1);
    gpio_set_direction(pin, GPIO_MODE_OUTPUT);
    gpio_hal_iomux_func_sel(GPIO_PIN_MUX_REG[pin], PIN_FUNC_GPIO);
    gpio_set_direction(pin+1, GPIO_MODE_INPUT);
    gpio_hal_iomux_func_sel(GPIO_PIN_MUX_REG[pin+1], PIN_FUNC_GPIO);
    ...
    uint32_t i = 0;
    while(!gpio_get_level(pin+1) && i++ < 15) esp_rom_delay_us(100);
    ...
    ESP_ERROR_CHECK(uart_driver_install(1, 1024, 0, 0, NULL, 0));
    ESP_ERROR_CHECK(uart_set_rx_full_threshold(1, 1));
    ESP_ERROR_CHECK(uart_param_config(1, &uart_config));
    ESP_ERROR_CHECK(uart_set_mode(1, UART_MODE_RS485_HALF_DUPLEX));
    ESP_ERROR_CHECK(uart_set_pin(1, pin, pin+1, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
    gpio_set_pull_mode(pin+1, GPIO_FLOATING);
...
bool UPDI_Program(gpio_num_t pin, uint8_t *data, uint32_t length)
{
    ESP_LOGI("UPDI", "programming attiny");
    UPDI_ERROR_CHECK(UPDI_Enable(pin));
